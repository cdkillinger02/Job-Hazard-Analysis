...
def get_jha(jha_id: int, db: Session = Depends(get_db)):
    jha = (
        db.query(models.JobHazardAnalysis)
        .filter(models.JobHazardAnalysis.id == jha_id)
        .first()
    )
    if not jha:
        ...
        "steps": [
            {
                ...
                "photo": step.photo,
                "photoBase64Encoded": (
                    crud.getBase64EncodedImage(
                        "http://127.0.0.1:8000/api/uploads/" + step.photo, step.photo
                    )
                    if step.photo
                    else ""
                ),
    ...
    if "signatures" in jha_data:
        ...
        for sig_data in jha_data["signatures"]:
            sig = models.Signature(
                name=sig_data.get("name", ""), date=sig_data.get("date"), jha_id=jha.id
            )
            db.add(sig)
...
def delete_jha(jha_id: int, db: Session = Depends(get_db)):
    ...
    db.delete(
        jha
    )  # This also deletes steps, hazards, and controls if cascade="all, delete-orphan" is set
    db.commit()
